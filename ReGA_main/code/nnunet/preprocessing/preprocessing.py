...
def resample_data_or_seg(data, new_shape, is_seg, axis=None, order=3, do_separate_z=False, cval=0, order_z=0):
    ...
    if is_seg:
        ...
        resize_fn = resize
        kwargs = {'mode': 'edge', 'anti_aliasing': False, 'cval': cval}  # 仅对图像数据传递 cval
        ...
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                ...
                if shape[axis] != new_shape[axis]:
                    ...
                    row_scale = np.float32(orig_rows) / rows
                    col_scale = np.float32(orig_cols) / cols
                    dim_scale = np.float32(orig_dim) / dim

                    # 修改后：显式指定为 float32 以节省一半内存
                    map_rows, map_cols, map_dims = np.mgrid[:rows, :cols, :dim].astype(np.float32)
                    map_rows *= row_scale
                    map_rows += (row_scale * 0.5 - 0.5)

                    map_cols *= col_scale
                    map_cols += (col_scale * 0.5 - 0.5)

                    map_dims *= dim_scale
                    map_dims += (dim_scale * 0.5 - 0.5)

                    coord_map = np.array([map_rows, map_cols, map_dims])
                    # coord_map = np.stack([map_rows, map_cols, map_dims]).astype(np.float32)

                    # 关键：手动删除不再需要的中间变量以释放内存空间
