...
from pymic.util.hd_calculate import compute_hd_multi_class
import numpy as np
from pymic.util.postprocessing import newsdf_post_processor  # 确保这个函数在你的 postprocessing.py 中可用


def post_process_sdf(pred, num_classes=4, sdf_th=35, region_th=2000):
    pred_post = pred.copy()

    for c in range(1, num_classes + 1):
        # 1) 用 pred_post（逐类顺序继承修改）
        binary_mask = (pred_post == c).astype(np.uint8)
        orig_cnt = int(binary_mask.sum())
        if orig_cnt == 0:
            continue

        # 2) 二值SDF后处理（返回仍是0/1）
        processed_binary = newsdf_post_processor(
            binary_mask,
            sdf_th=sdf_th,
            region_th=region_th
        )

        keep_mask = (processed_binary > 0)

        # 3) 只在“当前类区域”内做删/留，绝不覆盖其它类
        pred_post[(pred_post == c) & (~keep_mask)] = 0
        pred_post[(pred_post == c) & keep_mask] = c  # 可省略，但保留更直观

        keep_cnt = int(keep_mask.sum())
        print(f"[SDF] class {c}: orig={orig_cnt}, keep={keep_cnt}, removed={orig_cnt - keep_cnt}")
...
def get_binary_evaluation_score(s_volume, g_volume, spacing, metric):
    ...
    if (metric_lower == "dice"):
        ...
        # score = compute_hd_multi_class(s_volume, g_volume, num_classes=4)
