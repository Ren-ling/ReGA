...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x, num_repeats, BATCH_SIZE=None, tile_in_z=True, step=2,
        ...
        with torch.no_grad():
            ...
            # 将输入xpad到能被patch整除的尺寸（避免边缘无法覆盖）
            # slicer记录原始区域位置，最后用于裁剪回原尺寸

            data = data[None]  # 添加 batch 维度 → [1, C, D, H, W]
            ...
            # 用一个全零 patch 做一次前向，获取输出通道数

            # 构建高斯权重 (融合时 中心贡献大 边缘平滑过渡) 创建一个中心为1且向边缘逐渐衰减的高斯分布，并将其归一化。如果不用高斯加权，则所有位置的权重都设为1
            if use_gaussian:
            ...
            # data, result and add_torch and result_num samples are now on GPU
            for x in xsteps:
            ...
            result = result[slicer]  # 去掉 padding
            result_numsamples = result_numsamples[slicer]

            softmax_pred = result / result_numsamples  # 加权平均 → 平滑概率图
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x, num_repeats, mirror_axes, do_mirroring=True, mult=None):
        ...
        # 在测试/推理阶段，通过对输入体积做 8 种镜像翻转（左右、上下、前后），分别预测后再翻回来取平均
        with torch.no_grad():
            ...
            for i in range(num_repeats):
                for m in range(mirror_idx):
                    ...
                    if m == 1 and (2 in mirror_axes):  # Flip along z-axis
                        output = self(flip(x, 4))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(pred, 4)

                    if m == 2 and (1 in mirror_axes):  # # Flip along y-axis
                        output = self(flip(x, 3))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(pred, 3)

                    if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):  # Flip along z and y axes
                        output = self(flip(flip(x, 4), 3))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(flip(pred, 4), 3)

                    if m == 4 and (0 in mirror_axes):  # Flip along x-axis
                        output = self(flip(x, 2))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(pred, 2)

                    if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):  # Flip along x and z axes
                        output = self(flip(flip(x, 4), 2))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(flip(pred, 4), 2)

                    if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):  # Flip along x and y axes
                        output = self(flip(flip(x, 3), 2))
                        pred = self.inference_apply_nonlin(output[0] if isinstance(output, tuple) else output)
                        result_torch += 1 / num_results * flip(flip(pred, 3), 2)

                    if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):  # Flip along x,
                        # y and z axes
