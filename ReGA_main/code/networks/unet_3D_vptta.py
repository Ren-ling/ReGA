...
try:
    ...
    # 现在导入
    from convert_3d import *
except ImportError:
    # 如果还是失败，创建一个空的占位符
    print("Warning: Could not import from utils.convert_3d in unet_3D_vptta.py")
    # 定义一些空的东西，让代码继续运行
    AdaBN = None
...
class unet_3D_vptta(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, is_batchnorm=True, convert=True, newBN=AdaBN, warm_n=5):
        super(unet_3D_vptta, self).__init__()
        ...
        if convert:
            self.encoder = replace_bn_with_adabn(self.encoder, newBN, warm_n=warm_n)
            self.main_decoder_1 = replace_bn_with_adabn(self.main_decoder_1, newBN, warm_n=warm_n)
