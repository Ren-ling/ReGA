...
def load_case_from_list_of_files(data_files, seg_file=None):
    ...
    data_npy = np.vstack([sitk.GetArrayFromImage(d)[None] for d in data_itk])  # shape (d, h, w) will change to (1, d,
    # h, w) with [None]. Then come to (modalities, d, h, w)
...
def get_patient_identifiers_from_cropped_files(folder):
    return [i.split("/")[-1][:-4] for i in
            subfiles(folder, join=True, suffix=".npz")]  ### ...... fei zhe jin gan sha....
...
class ImageCropper(object):
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        on the cluster
        :param list_of_files: list of files [[PATIENTID_TIMESTEP_0000.nii.gz], [PATIENTID_TIMESTEP_0000.nii.gz]]
