...
class BaseDataSets(Dataset):
    def __init__(
        ...
        if self.split == "train":
            ...
            with open(self._base_dir + "/" + self.split + ".list", "r") as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
        if num is not None and "train" in self.split:
...
class SSLDataSets(Dataset):
    def __init__(
        ...
        for dataset in self.datasets:
            if self.split == "train":
                with open(self._base_dir + '/' + dataset + "/train.list", "r") as f1:
            ...
            # elif self.split == "val":
            #     with open(self._base_dir + '/' + dataset + "/val.list", "r") as f:
            #         self.sample_list = f.readlines()
            #     self.sample_list = [item.replace("\n", "") for item in self.sample_list]
            # else:
            #     with open(self._base_dir + "/" + self.split + ".list", "r") as f1:
        ...
        print(" {}, total {} samples from {}".format(self.split, len(self.sample_list), self.datasets))
    ...
    def __getitem__(self, idx):
        ...
        # print(sample['label'].shape, sample['label'].max(), sample['label'].min(),'lab')
        return sample
...
class RandomGenerator3d(object):
    ...
    def __call__(self, sample):
        ...
        #     image, label = random_rotate(image, label)
        # xx, x, y = image.shape
        # image = zoom(image, (1, self.output_size[0] / x, self.output_size[0] / y), order=1)
        # label = zoom(label, (1, self.output_size[0] / x, self.output_size[0] / y), order=1)
