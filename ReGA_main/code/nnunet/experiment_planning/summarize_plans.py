...
if __name__ == "__main__":
    ...
    task_dirs = [i for i in subdirs(preprocessing_output_dir, join=False, prefix="Task") if
                 i.find("BrainTumor") == -1 and i.find("MSSeg") == -1]
    ...
    with open("2019_02_06_plans_summary.csv", 'w') as f:
        f.write(
            "task;plans_file;stage;batch_size;num_pool_per_axis;patch_size;patch_size("
            "mm);median_patient_size_in_voxels;median_patient_size_in_mm;current_spacing;original_spacing"
            ";pool_op_kernel_sizes;conv_kernel_sizes\n")
        for t in task_dirs:
            print(t)
            tmp = join(preprocessing_output_dir, t)
            plans_files = [i for i in subfiles(tmp, suffix=".pkl", join=False) if
                           i.find("_plans_") != -1 and i.find("Dgx2") == -1]
