...
def get_caseIDs_from_splitted_dataset_folder(folder):
    ...
    files = [i[:-12] for i in files]  # 从每个文件名末尾去掉最后 12 个字符，剩下的就是 case_id
...
if __name__ == "__main__":
    ...
    if not use_splitted or not isdir(
            join(splitted_4d_output_dir, task)):
        print("splitting task ", task)
        split_4d(task)
    # 如果我们不需要使用已有的 splitted 数据，或者 splitted 目录根本不存在，就重新运行 split_4d 来拆分 4D 数据

    crop(task, override=override,
         num_threads=processes_lowres)  # got cropped data and seg(with -1) in one array (m+1, d, h, w); and pkl with
    # properties
