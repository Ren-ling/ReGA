...
def get_reasonable_crops_of_lumbar(gt3d, patch_size, stage):
    ...
    lambar = np.where(gt3d == 4, 1, 0)  # 腰椎（label 4）
    sacral = np.where(gt3d == 1, 1, 0)  # 骶骨（label 1）
    upside_down = False
    ...
    if d_sacral.mean() > d_lambar.mean():
        upside_down = True
        print(upside_down)
        # 正常人体：腰椎在上面，骶骨在下面 → 腰椎D坐标 < 骶骨D坐标 倒过来了
...
def main_3d(base_path, check_save_path):
    ...
    names = [i[:-4] for i in names if i.endswith('.npz')]
    ...
    pool.close()
    pool.join()
    # 每个case调用一次_main_3d_one_case，所有固定参数已通过partial绑定
