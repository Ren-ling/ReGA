...
# 过采样前景（foreground oversampling），特别是在训练时需要更多关注小而不平衡的目标区域
# has_prev_stage：是否存在上一阶段的分割结果（级联网络中使用，上一阶段的输出作为输入之一）
class DataLoader3D_oversampleJoint(DataLoader3D):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            case_properties.append(self._data[i]['properties'])
            npz_path = self._data[i]['data_file'][:-4] + ".npz"
            npy_path = self._data[i]['data_file'][:-4] + ".npy"
            if isfile(npz_path):
                case_all_data = np.load(npz_path)['data']
            elif isfile(npy_path):
                case_all_data = np.load(npy_path, allow_pickle=True)
            else:
                raise FileNotFoundError(f"Neither .npz nor .npy found for {self._data[i]['data_file']}")
