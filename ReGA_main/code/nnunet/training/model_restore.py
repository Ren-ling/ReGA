...
def recursive_find_trainer(folder, trainer_name, current_module):
    # folder：一个包含单个路径的列表，指定要搜索的目录（例如['/path/to/nnunet/training/network_training']）。
    # trainer_name：要查找的类名（字符串，例如
    # 'nnUNetTrainer'）。
    # current_module：当前模块的名称（字符串，例如
    # 'nnunet.training.network_training'），用于动态导入模块。
    tr = None
