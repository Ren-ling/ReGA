...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
    ...
    # trainer还是nnUNetTrainer，只是它内部会根据stage（0 → lowres，1 → fullres）来训练不同分辨率的网络

    trainer_class = recursive_find_trainer([join(*search_in)], network_trainer, current_module=base_module)
    # trainer_class 是通过 recursive_find_trainer 去 nnunet/training/network_training 下找 nnUNetTrainer 这个类
    ...
    if (network == '2d' or len(possible_stages) > 1) and not network == '3d_lowres':
        ...
        # 当网络是 2D 网络（network == '2d'）或者有 多个阶段（len(possible_stages) > 1），
        # 并且网络 不是低分辨率的 3D 网络（not network == '3d_lowres'）时，batch_dice 会被设置为 True
    else:
        batch_dice = False
