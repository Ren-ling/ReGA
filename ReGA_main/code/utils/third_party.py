...
def shear_x(pil_img, level):
    ...
    if np.random.uniform() > 0.5:
        level = -level
    return pil_img.transform((320, 320), Image.AFFINE, (1, level, 0, 0, 1, 0), resample=Image.BILINEAR, fillcolor=128)

def shear_y(pil_img, level):
    level = float_parameter(rand_lvl(level), 0.3)
    if np.random.uniform() > 0.5:
        level = -level
    return pil_img.transform((320, 320), Image.AFFINE, (1, 0, 0, level, 1, 0), resample=Image.BILINEAR, fillcolor=128)

def translate_x(pil_img, level):
    level = int_parameter(rand_lvl(level), 320 / 3)
    if np.random.random() > 0.5:
        level = -level
    return pil_img.transform((320, 320), Image.AFFINE, (1, 0, level, 0, 1, 0), resample=Image.BILINEAR, fillcolor=128)

def translate_y(pil_img, level):
    level = int_parameter(rand_lvl(level), 320 / 3)
    if np.random.random() > 0.5:
        level = -level
    return pil_img.transform((320, 320), Image.AFFINE, (1, 0, 0, 0, 1, level), resample=Image.BILINEAR, fillcolor=128)
